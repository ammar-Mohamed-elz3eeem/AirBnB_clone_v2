...
def number_odd_or_even_route(n):
    """function to handle requests made to /number_odd_or_even/<int:n>
    route and show template for that route using render template func
    with custom filter that filters numbers according to if it is odd
    or even
    ...
    Returns:
        rendering the template of the number html page
        with function showing number if odd or even
    """
    return render_template("6-number_odd_or_even.html",
                           number=n, filter_numbers=is_even)
